...
<#
.SYNOPSIS
Final release. Audits and optionally repairs vSphere Native Key Provider host configuration, then creates an Excel workbook with provider, host, key, and object usage tabs.
...
The workbook contains Summary, Providers, Host Status, Key Usage, All Keys, and Errors tabs. CSV copies are
also written to a companion directory. No plaintext cryptographic key material is read or exported.

.EXAMPLE
./NKPAuditandRepairRevA..ps1 -vCenter vcsa01.example.com -InstallPrerequisites

.EXAMPLE
./NKPAuditandRepairRevA..ps1 -vCenter vcsa01.example.com -Cluster Cluster01 -KeyProvider DEV1 -Remediate -PropagationSeconds 300

.EXAMPLE
./NKPAuditandRepairRevA..ps1 -vCenter vcsa01.example.com -Cluster Cluster01 -KeyProvider DEV1 -Remediate -WhatIf
#>
...
try {
    ...
    foreach ($item in $workItems) {
        if (-not $Remediate -or $item.Initial.Compliant) {
    ...
    foreach ($item in $workItems) {
        ...
        $status = if ($item.Initial.Compliant) { 'AlreadyHealthy' }
